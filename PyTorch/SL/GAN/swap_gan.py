    ...
    def train_batch(self, data):
        ...
        if(self.summary is not None):
            ...
            self.summary.add_scalar("Discriminator Loss", np_disc_loss,
                                    self.num_batches)
            self.summary.add_scalar("Discriminator Real Loss", disc_real_loss.item(),
                                    self.num_batches)
            self.summary.add_scalar("Discriminator Fake Loss", disc_fake_loss.item(),
                                    self.num_batches)
...
def swap(image, other, block_size):
    """
    ...
    # The block indices
    hb = torch.randint(rand_bound, (1,)).item()
    wb = torch.randint(rand_bound, (1,)).item()

    # Get the target tensors
    ones_target = torch.ones(image.shape[0], bs, bs)
    img_targ = torch.ones(image.shape[0], bs, bs)
    ...
    return cloned_img, cloned_other, img_targ, other_targ, ones_target
